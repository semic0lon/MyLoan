...
from .models import Loan, Payment, Loan2, Payment2
from . import db
import subprocess
...
def loan2():
    ...
    loans = Loan2.query.order_by(Loan2.loan_date.desc()).all()

    data = [enrich_loan2(l) for l in loans]

    return render_template("loan2.html", loans=data)
...
def manage2():
    ...
    loans = Loan2.query.order_by(Loan2.loan_date.desc()).all()
    data = [enrich_loan2(l) for l in loans]
    return render_template("manage2.html", loans=data)
...
@main.route("/loan2/new", methods=["GET", "POST"])
def new_loan2():

    if request.method == "POST":

        loan_date = datetime.strptime(
            request.form["loan_date"], "%Y-%m-%d"
        ).date()
        ...
        return redirect(url_for("main.manage2"))  # กลับไปหน้า manage2

    return render_template("new_loan2.html")
...
@main.route("/loan2/<int:loan_id>/edit", methods=["GET", "POST"])
def edit_loan2(loan_id):

    loan = Loan2.query.get_or_404(loan_id)

    if request.method == "POST":

        amount = float(request.form["amount"])
        pay_date = datetime.strptime(
            request.form["pay_date"], "%Y-%m-%d"
        ).date()
        ...
        return redirect(
            url_for("main.edit_loan2", loan_id=loan.id)
        )

    status = loan.calculate_status()

    payments = Payment2.query.filter_by(
        loan_id=loan.id
    ).order_by(Payment2.pay_date.desc()).all()
...
@main.route("/loan2/<int:loan_id>/delete", methods=["POST"])
def delete_loan2(loan_id):
    loan = Loan2.query.get_or_404(loan_id)

    db.session.delete(loan)
    db.session.commit()

    return redirect(url_for("main.manage2"))
